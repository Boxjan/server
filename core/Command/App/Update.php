...
class Update extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		foreach ($apps as $appId) {
			$newVersion = $this->installer->isUpdateAvailable($appId, $input->getOption('allownightly'));
			if ($newVersion) {
				$output->writeln($appId . ' new version available: ' . $newVersion);

				if (!$input->getOption('showonly')) {
					try {
						$result = $this->installer->updateAppstoreApp($appId, $input->getOption('allownightly'));
					} catch (\Exception $e) {
